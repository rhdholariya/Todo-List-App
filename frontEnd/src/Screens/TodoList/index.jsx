...
const TodoList = () => {
    ...
    const [showRemoveModal, setShowRemoveModal] = useState({show: false, data: null});
    ...
    const handleComplete = (data, value) => {
        putData(`${baseUrl}/${data._id}`, {isCompleted: value}).then(() => {
            setTodos(todos.map(todo => (
                todo._id === data._id ? {...todo, isCompleted: !todo.isCompleted} : todo
            )));
        }).catch((err) => {
    ...
    const handleEdit = (data, newText) => {
        setEditing(!editing);
        putData(`${baseUrl}/${data._id}`, {task: newText}).then((res) => {
            getData("").then(response => {
                setTodos(response.todoList);
            }).catch(()=>{})
    ...
    const handleDelete = (id) => {
        deleteData(`${baseUrl}/${id}`).then((res) => {
            getData("").then(response => {
                setTodos(response.todoList);
            }).catch(()=>{})
            toast.success(res.message);
    ...
    const addList = () => {
        let data  = (todos || []);
        postData(baseUrl, {task: addValue.trim()}).then(response => {
    ...
    return (
        <>
            ...
            <div className="d-flex justify-content-center">
                <HeroSection todos_completed={todos_completed} total_todos={(todos || []).length}/>
            </div>
            <Container>
